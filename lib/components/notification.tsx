        ...
        for (let attempt = 1; attempt <= MCP_MAX_CONNECT_ATTEMPTS; attempt++) {
            try {
                ...
                const results = await Promise.allSettled([
                    aiService.callTool("generateAdaptiveVideoQuizzes", {
                        token: params.token,
                    ...
                    aiService.callTool("generateFullAssessment", {
                        token: params.token,
                        userId: params.userId,
                        videoId: params.videoId,
        ...
        eventSource.onmessage = async (event: MessageEvent) => {
            ...
            switch (data.event) {
                ...
                case "evaluation_started": {
                    if (data.video_id) {
                        window.dispatchEvent(
                            new CustomEvent("evaluationStarted", {
                                detail: { videoId: String(data.video_id) },
                            })
                        )
                    }
                    break
                ...
                case "evaluation_failed": {
                    if (data.video_id) {
                        window.dispatchEvent(
                            new CustomEvent("evaluationFailed", {
                                detail: { videoId: String(data.video_id), message: data.message },
                            })
                        )
                    }
                    break
